...
/**
 * A WindowUtil that uses xprop/xwininfo to get the active window bounds.
 * Note: This does not include the title bar, which is why GnomeWindowUtil is preferred...
 * However, GnomeWindowUtil does not have the actual window title, however it is currently not used.
 * 
 * @author Nikki
 *
 */
public class XPropWindowUtil implements WindowUtil {
	 ...
	 */
	private static Pattern pattern = Pattern.compile("Width:\\s*(\\d+)\\s*Height:\\s*(\\d+)");
	
	/**
	 * The pattern to match the window location (Does not include title bar)
	 */
	private static Pattern locationPattern = Pattern.compile("Absolute upper-left X:\\s*(\\d+)\\s*Absolute upper-left Y:\\s*(\\d+)");
	
	/**
	 * The pattern to match the window name
	 */
	private static Pattern namePattern = Pattern.compile("xwininfo\\: Window id\\: .*? \"(.*?)\"");
	
	/**
	 * The pattern to match the window id from the xprop -root command
	 */
	private static Pattern windowid = Pattern.compile("_NET_ACTIVE_WINDOW\\(WINDOW\\): window id # (.*)");
	 ...
	 * @return
	 * 			The output of the comman
	 * @throws IOException
	 * 			If an error occurred while reading from the process
	 * @throws InterruptedException
	 * 			If the process is interrupted while waiting
	 */
	public static String runProcess(String cmdline) throws IOException, InterruptedException {
		Process p = Runtime
				.getRuntime()
				.exec(cmdline);
		p.waitFor();
